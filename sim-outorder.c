		  ...
		  int argc, char **argv)        /* command line arguments */
{
  ...
  if (fastfwd_count < 0 || fastfwd_count >= (unsigned) 4294963790)
    fatal("bad fast forward count: %u", fastfwd_count);
  ...
  cache_leak_ctrl_check_options(odb, argc, argv);
...
sim_reg_stats(struct stat_sdb_t *sdb)   /* stats database */
{
 ...
 /* register baseline stats */
...
#ifdef KAXIRAS_LASTUSE
  stat_reg_counter(sdb, "predicts",
		   "total last access predictions made",
		   &predicts, /* initial value */0, /* format */NULL);
  stat_reg_counter(sdb, "mis_predicts",
		   "total last access mis_predictions made",
		   &mis_predicts, /* initial value */0, /* format */NULL);
  stat_reg_formula(sdb, "last_access_missrate",
		   "miss rate for last access prediction",
		   "mis_predicts / predicts", /* format */NULL);
  stat_reg_counter(sdb, "uncounted_loads",
		   "uncounted loads",
		   &uncounted_loads, /* initial value */0, /* format */NULL);
  stat_reg_counter(sdb, "uncounted_writes",
		   "uncunted writes",
		   &uncounted_writes, /* initial value */0, /* format */NULL);
  stat_reg_counter(sdb, "predict_not_last_uses",
		   "predicted # of not last uses",
		   &predict_not_last_uses, /* initial value */0, /* format */NULL);
  stat_reg_counter(sdb, "predict_last_uses",
		   "predicted # of last uses",
		   &predict_last_uses, /* initial value */0, /* format */NULL);
  stat_reg_counter(sdb, "verified_last_uses",
		   "verified_last_uses",
		   &verified_last_uses, /* initial value */0, /* format */NULL);
  stat_reg_counter(sdb, "verified_not_last_uses",
		   "verified_not_last_uses",
		   &verified_not_last_uses, /* initial value */0, /* format */NULL);
#endif /* KAXIRAS_LASTUSE */
  ...
  /* Leakage: register stats */
    cache_leak_ctrl_reg_stats (sdb);
...
/* compute leakage power estimates */
...
sim_uninit(void)
{
	...
	//sim_num_insn -= fastfwd_count;

	/* Leakage: uninitialization	*/
	cache_leak_ctrl_uninit();
  ...
  for (n_issued=0;
      ...
      if (RSLINK_VALID(node))
	{
	  ...
	  if (rs->in_LSQ
	    ...
	    {
#ifdef KAXIRAS_LASTUSE
               predict_store((unsigned int)rs->PC,(unsigned int)rs->addr,(unsigned int)sim_cycle);
               /** printf("s %x to %x\n",rs->PC,rs->addr); **/
#endif /* KAXIRAS_LASTUSE */
...
#ifdef KAXIRAS_LASTUSE
               if(predict_load((unsigned int)rs->PC,(unsigned int)rs->addr,(unsigned int)sim_cycle))
               /** printf("l %x to %x\n",rs->PC,rs->addr); **/
#endif /* KAXIRAS_LASTUSE */
...
#include "machine.def"
      ...
      if (!spec_mode)
	{
	  ...
	  if (MD_OP_FLAGS(op) & F_CTRL)
		...
		{
#ifndef WATTCH_ID_BPRED_ACCESS_BUG_COMPAT
             /* Wattch -- bpred access */
             bpred_access++;
#endif
		  bpred_update(pred,
...
#if defined(ACCESS_CACHE_WHEN_FASTFWD)
...
#endif /* defined(ACCESS_CACHE_WHEN_FASTFWD) */
...
#if defined(ACCESS_CACHE_WHEN_FASTFWD)
      /* access instruction TLB and cache */
      if (itlb)
...
#include "machine.def"
  ...
  fastfwd_count = sim_num_insn;
  skip_start += fastfwd_count;
  max_insts += skip_start;

  /* clear the cache stats */
  if (cache_il1 && (cache_il1 != cache_dl1 && cache_il1 != cache_dl2))
    clear_cache_stats(cache_il1);
  if (cache_il2 && (cache_il2 != cache_dl1 && cache_il2 != cache_dl2))
    clear_cache_stats(cache_il2);
  if (cache_dl1)
    clear_cache_stats(cache_dl1);
  if (cache_dl2)
    clear_cache_stats(cache_dl2);
  if (itlb)
    clear_cache_stats(itlb);
  if (dtlb)
    clear_cache_stats(dtlb);
  ...
  for (;;)
	...
	if (!b_decay_enabled && sim_num_insn >= skip_start)
		if (global_counter_max || local_counter_max || fast_counter || slow_counter)
		  b_decay_enabled = TRUE;

	  /* note: even when we don't do decay, we still need the profiles */
	if (!b_decay_profile_enabled && sim_num_insn >= skip_start)
	{
	  b_decay_profile_enabled = TRUE;
	}

	  /* update cache decay every global_counter_tick */
	if (b_decay_enabled && sim_cycle % global_counter_max == 0)
		update_cache_decay();

	  /* update the cache stats every sample_interval  */
    if (b_decay_profile_enabled && sim_cycle % sample_interval == 0)
		update_cache_stats();
