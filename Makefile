...
#
#FFLAGS = -DDEBUG
FFLAGS = -Ddecayed_cache=cache_dl1
#FFLAGS =  
...
SRCS =	main.c sim-fast.c sim-safe.c sim-cache.c sim-profile.c \
	...
	eio.c stats.c endian.c misc.c powerinit.c power.c wattch-interact.c leakage.c leakinit.c cache_leak_ctrl.c \
...
HDRS =	syscall.h memory.h regs.h sim.h loader.h cache.h  bpred.h ptrace.h \
	...
	eio.h range.h version.h endian.h misc.h power.h  leakage.h cache_leak_ctrl.h\
...
#
all: sim-outorder$(EEXT)
	@echo "my work is done here..."

symlinks:
	-$(RM)	target-alpha$(X)sysprobe target-pisa$(X)sysprobe \
			tests-alpha$(X)sysprobe tests-pisa$(X)sysprobe \
			libcheetah$(X)sysprobe libexo$(X)sysprobe cacti$(X)sysprobe
	$(LN) ..$(X)sysprobe target-alpha$(X)sysprobe
	$(LN) ..$(X)sysprobe target-pisa$(X)sysprobe
	$(LN) ..$(X)sysprobe tests-alpha$(X)sysprobe
	$(LN) ..$(X)sysprobe tests-pisa$(X)sysprobe
	$(LN) ..$(X)sysprobe libcheetah$(X)sysprobe
	$(LN) ..$(X)sysprobe libexo$(X)sysprobe
	$(LN) ..$(X)sysprobe cacti$(X)sysprobe
...
wattch-interact$(EEXT): sysprobe$(EEXT) eval.$(OEXT) misc.$(OEXT) \
			stats.$(OEXT) power.$(OEXT) wattch-interact.$(OEXT) cacti/libcacti.$(LEXT)
	$(CC) -o wattch-interact$(EEXT) $(CFLAGS) eval.$(OEXT) misc.$(OEXT) stats.$(OEXT) \
		power.$(OEXT) wattch-interact.$(OEXT) cacti/libcacti.$(LEXT) $(MLIBS)
...
sim-outorder$(EEXT):	sysprobe$(EEXT) sim-outorder.$(OEXT) cache.$(OEXT)  bpred.$(OEXT) resource.$(OEXT) ptrace.$(OEXT) powerinit.$(OEXT) power.$(OEXT) leakinit.$(OEXT) cache_leak_ctrl.$(OEXT) leakage.$(OEXT) $(OBJS) libexo/libexo.$(LEXT) cacti/libcacti.$(LEXT) 
	$(CC) -o sim-outorder$(EEXT) $(CFLAGS) sim-outorder.$(OEXT) cache.$(OEXT)  bpred.$(OEXT) resource.$(OEXT) ptrace.$(OEXT) powerinit.$(OEXT) power.$(OEXT) cache_leak_ctrl.$(OEXT) leakinit.$(OEXT) leakage.$(OEXT) $(OBJS) libexo/libexo.$(LEXT) cacti/libcacti.$(LEXT) $(MLIBS)
...
clean:
	-$(RM) *.o *.obj core *~ Makefile.bak sysprobe$(EEXT) $(PROGS) wattch-interact$(EEXT)
...
leakclean:
	$(RM) cache.$(OEXT) cache_leak_ctrl.$(OEXT) leakage.$(OEXT) powerinit.$(OEXT) leakinit.$(OEXT) sim-outorder.$(OEXT)
unpure:
...
endian.$(OEXT): endian.h loader.h host.h misc.h machine.h machine.def regs.h
leakinit.$(OEXT): leakage.h power.h stats.h options.h
powerinit.$(OEXT): power.h stats.h options.h leakage.h
endian.$(OEXT): memory.h options.h stats.h eval.h
